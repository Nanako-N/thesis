...
def Destination(route_map, nTown, src, dst, lineNum):
    ...
    joinpathlist = [] #joinmeetに行くための経路のリスト
    ...
    if src[0][0]!=1: #重複があるとき
        if lineNum==1: #全員同路線
            for i in src[0][1]:
                ...
                if d<maindis:
                    ...
                    via = v
            kaisatu[0] = M
            joinpathlist.append(getPath(dst,via))
            nearpathlist.append(getPath(dst,via))
            print("joinpathlist : "+str(joinpathlist))
            print("nearpathlist : "+str(nearpathlist))
            meet.extend([[M],[M]])
            return (meet,kaisatu,joinpathlist,nearpathlist)
        if src[0][0]==src[1][0]==2:
            for n in range(lineNum):
                ...
                for i in src[n][1]:
                    ...
                    if d<maindis:
                        ...
                        mainvia = v
                        mainline = n
            joinpathlist.append(getPath(dst,mainvia))
            ...
            for i in other:
                m,d,v = meetDist(route_map, nTown, i, dst, mainvia)
                joinpathlist.append(getPath(m,v))
                joinmeet.append(m)
            for m in joinmeet: #目的地に最も近い待ち合わせ場所を算出
                d,v = solve(route_map, nTown, m, dst)
                if d<neardst:
                    neardst = d
                    nearmeet = m
            for i in other:
                d,v = solve(route_map, nTown, i, nearmeet)
        ...
        print("改札番号： "+str(kaisatu))
        print("目的地あり:　"+str(other))
        for i in other: #合流方式の待ち合わせ場所の算出
            m,d,v = meetDist(route_map, nTown, i, dst, mainvia)
            joinpathlist.append(getPath(m,v))
            joinmeet.append(m)
        for n in joinmeet: #目的地に最も近い待ち合わせ場所を算出
            d,v = solve(route_map, nTown, n, dst)
            if d<neardst:
                neardst = d
                nearmeet = n
        for i in other:
...
def noDestination(route_map, nTown, src, lineNum):
    ...
    sumdis = sys.maxsize
    joinpathlist = [] #nearmeetに行くための経路のリスト
    ...
    if src[0][0]!=1: #重複している
        if lineNum==1: #全員が同じ路線の時→改札を返す
            for i in src[0][1]:
                joinmeet.append(i)
                kaisatu.append(i)
                joinpathlist.append([i])
                onepathlist.append([i])
            meet.extend([joinmeet,joinmeet])
            return (meet,kaisatu,joinpathlist,onepathlist)
        if lineNum==2: #使用路線数が2→人数の多い路線の改札に集合するとき
            if src[0][0]==2 and src[1][0]==2: #4人.2.2
                ...
                joinpathlist.append(getPath(path[index],mainvia))
                joinpathlist.append(getPath(path[index],v))
            ...
            for i in src[0][1]:
                for j in src[1][1]:
                    ...
                    if d<maindis: #メインルートなら
                        ...
                        M = i
                        mainvia = v
            joinpathlist.append(getPath(j,mainvia))
            onepathlist.append(getPath(j,mainvia))
            kaisatu.extend([start,goal])
            meet.extend([[M],[M]])
            return (meet,kaisatu,joinpathlist,onepathlist)
                        ...
                        if d<maindis:
                            ...
                            for l in range(lineNum):
                                if l!=0 and l!=n+1:
                                    other.append(src[l][1])
        #d,v = solve(route_map,nTown,goal,path[index])
        joinpathlist.append(getPath(goal,mainvia))
        ...
        for n in range(len(other)):
            ...
            for i in other[n]:
                m,d,v= meetDist(route_map, nTown, i, goal, mainvia)
                if d<mindis:
                    mindis = d
                    M = m
                    kai = i #otherからmeetに行くときの最短の改札
                    via = v
            joinpathlist.append(getPath(M,via))
